...
### 등록 전 중복 확인 (필수)
```bash
gh issue list --state all --search "핵심 키워드" --limit 10
```
비슷한 이슈가 있으면 새로 만들지 말고 기존 이슈에 코멘트로 추가한다. closed면 사유를 읽고 reopen 여부 판단.
...
**작업자는 작성 세션의 대화를 못 본다.** 작성 시점에 아는 판단 근거를 전부 본문으로 옮긴다. 해당되는 항목은 필수:

| 항목 | 넣을 것 |
|---|---|
| 시점·경위 | 발견 날짜(YYYY-MM-DD), 출처 — CS/인터뷰/코드리뷰/데이터 조사 등 |
| 식별자 | **userId**, CS·채널톡 번호, bookId/packageId, 기기·OS·앱 버전, 커밋·PR·이슈 번호 |
| 증거 원문 | 에러/로그 원문 그대로, 유저 발언 직접 인용, 수치, 재실행 가능한 조회 쿼리 |
| 시도·배제한 것 | 검토 후 버린 가설, 임시 우회책과 한계 |
| 코드 진입점 | 파일 경로·심볼·문서 |
| 사실 vs 추정 | "확인됨/추정/미확인"을 명시적으로 구분. 모르는 건 지어내지 말고 미확인으로 |

**제출 전 자가 검증**: 이 대화를 못 본 세션이 이슈만 읽고 착수 가능한가? 작업자의 첫 질문 3개를 상상하고 본문에 답이 있는지 확인한다. (실물 예시: solve#511·#517·#509)
